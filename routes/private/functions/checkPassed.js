...
function checkPassed(passedOptions, req, res, callback) {
  ...
  if (options.setTask) {
    ...
    if (!isValidTaskDate(startDate))
      return res.json({ message: "Invalid start date" });
    if (!end) return res.json({ message: "No end date provided" });
    const endDate = new Date(end);
    if (!isValidTaskDate(endDate))
      return res.json({ message: "Invalid end date" });
    if (startDate >= endDate)
      return res.json({ message: "Start date must be earlier than end date" });
  }

  if (options.deleteTaskId) {
    const { deleteTaskId } = req.body;
    if (!deleteTaskId) return res.json({ message: "No task id provided" });
    if (!isValidObjectId(deleteTaskId))
      return res.json({ message: "Invalid task id" });
  }
